...
// 分布统计共享表格：行=token，列=动态桶（range_groups），单元格=count。
...
export function DistributionTable({
  ...
  // 按 token 聚合：每个 token 在各桶的 count 与总计。
  const { rows, tokenOrder } = useMemo(() => {
    ...
    for (const item of items as DistributionItem[]) {
      ...
      bucket.set(item.range_grp, (bucket.get(item.range_grp) ?? 0) + item.count)
    }
    return {
      rows: map,
      tokenOrder: order,
